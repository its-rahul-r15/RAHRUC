      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md flex flex-col items-center">
        ...
        <p className="mt-2 text-center text-sm text-secondary-text">
          {showOtpInput ? (
            ...
            <>
              Or{' '}
              <Link to="/login" onClick={clearError} className="font-medium text-orange-primary hover:text-orange-primary/80 transition-colors">
        ...
        <div className="bg-white py-8 px-4 border border-border-subtle sm:rounded-2xl sm:px-10 shadow-sm">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              >
                {isLoading ? <Loader className="w-5 h-5 animate-spin" /> : showOtpInput ? 'Verify Code & Register' : 'Send Code'}
              </button>
            </div>
          </form>
        </div>
